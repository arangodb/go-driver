...
type CollectionInfo struct {
	...
	Name string `arangodb:"name,omitempty"`
	// The status of the collection
	Status CollectionStatus `arangodb:"status,omitempty"`
	// The type of the collection
	Type CollectionType `arangodb:"type,omitempty"`
	// If true then the collection is a system collection.
	IsSystem bool `arangodb:"isSystem,omitempty"`
}
...
type CollectionProperties struct {
	...
	KeyOptions  struct {
		...
		AllowUserKeys bool `arangodb:"allowUserKeys,omitempty"`
	} `arangodb:"keyOptions,omitempty"`
	// NumberOfShards is the number of shards of the collection.
	// Only available in cluster setup.
	NumberOfShards int `arangodb:"numberOfShards,omitempty"`
	// ShardKeys contains the names of document attributes that are used to determine the target shard for documents.
	// Only available in cluster setup.
	ShardKeys []string `arangodb:"shardKeys,omitempty"`
	// ReplicationFactor contains how many copies of each shard are kept on different DBServers.
	// Only available in cluster setup.
	ReplicationFactor int `arangodb:"replicationFactor,omitempty"`
}
...
type CollectionStatistics struct {
	...
	Figures     struct {
		...
		DocumentReferences int64 `arangodb:"documentReferences,omitempty"`
		CompactionStatus   struct {
			...
			Message string `arangodb:"message,omitempty"`
			...
			Time time.Time `arangodb:"time,omitempty"`
		} `arangodb:"compactionStatus"`
		Compactors struct {
		...
		Dead struct {
			...
			Deletion int64 `arangodb:"deletion,omitempty"`
			// The total size in bytes used by all dead documents.
			Size int64 `arangodb:"size,omitempty"`
		} `arangodb:"dead"`
		Indexes struct {
		...
		WaitingFor string `arangodb:"waitingFor,omitempty"`
		Alive      struct {
		...
		LastTick int64 `arangodb:"lastTick,omitempty"`
		Journals struct {
