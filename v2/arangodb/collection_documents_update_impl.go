...
func (c collectionDocumentUpdate) UpdateDocumentsWithOptions(ctx context.Context, documents interface{}, opts *CollectionDocumentUpdateOptions) (CollectionDocumentUpdateResponseReader, error) {
	...
	case http.StatusCreated:
		fallthrough
	case http.StatusAccepted:
		return newCollectionDocumentUpdateResponseReader(&arr, opts, documentCount), nil
	default:
		return nil, shared.NewResponseStruct().AsArangoErrorWithCode(code)
	}
}

func newCollectionDocumentUpdateResponseReader(array *connection.Array, options *CollectionDocumentUpdateOptions, documentCount int) *collectionDocumentUpdateResponseReader {
	c := &collectionDocumentUpdateResponseReader{
		array:         array,
		options:       options,
