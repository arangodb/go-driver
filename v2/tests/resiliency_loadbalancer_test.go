...
//	  → nginx ingress
//	  → coordinator pod IP(s) via Endpoints
//	  → GET /_admin/status  (via client.GetServerStatus)
//	  → response.serverInfo.serverId  (e.g. CRDN-ppwdeatg)
//
// ClientIP sessionAffinity on the coordinator Service does not apply on this path —
// nginx connects to pod IPs directly. This test logs coordinator distribution only;
// it does not assert stickiness because ingress may balance per request or connection.
func TestResiliency_0_LoadBalancerCoordinatorDistribution(t *testing.T) {
	...
	withContextT(t, defaultTestTimeout, func(ctx context.Context, tb testing.TB) {
		...
		t.Run("shared HTTP/2 connection", func(t *testing.T) {
			...
			unique := uniqueCoordinatorIDs(ids)
		...
		t.Run("fresh HTTP/1 connection per request", func(t *testing.T) {
			...
			case 1:
				t.Logf(
					"All %d requests hit coordinator %s (valid ingress LB routing).",
					len(ids),
					unique[0],
				)
			default:
				t.Logf("Requests were distributed across %d coordinators.", len(unique))
			}
...
func uniqueCoordinatorIDs(values []string) []string {
