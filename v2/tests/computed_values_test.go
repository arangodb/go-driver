		...
		WithDatabase(t, client, nil, func(db arangodb.Database) {
			withContextT(t, defaultTestTimeout, func(ctx context.Context, tb testing.TB) {
				...
				t.Run("Create with ComputedValues", func(t *testing.T) {
					WithCollectionV2(t, db, &arangodb.CreateCollectionPropertiesV2{
						ComputedValues: &[]arangodb.ComputedValue{computedValue},
					}, func(col arangodb.Collection) {
				...
				t.Run("Update ComputedValues", func(t *testing.T) {
					WithCollectionV2(t, db, nil, func(col arangodb.Collection) {
						...
						err = col.SetPropertiesV2(ctx, arangodb.SetCollectionPropertiesOptionsV2{
							ComputedValues: &[]arangodb.ComputedValue{computedValue},
						})
						require.NoError(t, err)

						// Check if the computed value is in the list of computed values
						col, err = db.GetCollection(ctx, col.Name(), nil)
						require.NoError(t, err)
