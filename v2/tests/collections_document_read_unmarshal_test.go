...
func Test_CollectionDocumentsReadUnmarshalCollision(t *testing.T) {
...
func Benchmark_CollectionDocumentsReadUnmarshalWithCollision(b *testing.B) {
	WrapB(b, func(b *testing.B, client arangodb.Client) {
		WithDatabase(b, client, nil, func(db arangodb.Database) {
			WithCollection(b, db, nil, func(col arangodb.Collection) {
				withContextT(b, defaultTestTimeout, func(ctx context.Context, tb testing.TB) {
					type DocWithCode struct {
						Name  string `json:"name"`
						Error string `json:"error"`
						Code  string `json:"code"`
					...
					for i := range N {
						doc_with_code := DocWithCode{
							Name:  "DocWithCode",
							Code:  fmt.Sprintf("%d%03d", rng.Int()%1000, i),
							Error: "error",
						}
						meta_with_code, _ := col.CreateDocument(ctx, doc_with_code)
						metas = append(metas, Meta{Key: meta_with_code.Key})
					}

					b.Run("one_by_one", func(b *testing.B) {
						var docRead DocWithCode
						for i := range N {
							_, _ = col.ReadDocumentWithOptions(ctx, metas[i].Key, &docRead, nil)
						}
					})

					b.Run("bulk", func(b *testing.B) {
						reader, _ := col.ReadDocumentsWithOptions(ctx, &metas, nil)
						var docRead DocWithCode
						for range N {
							_, _ = reader.Read(&docRead)
						}
					})
				})
			})
...
func Benchmark_CollectionDocumentsReadUnmarshalWithoutCollision(b *testing.B) {
	WrapB(b, func(b *testing.B, client arangodb.Client) {
		WithDatabase(b, client, nil, func(db arangodb.Database) {
			WithCollection(b, db, nil, func(col arangodb.Collection) {
				withContextT(b, defaultTestTimeout, func(ctx context.Context, tb testing.TB) {
					type DocWithCode struct {
						Name  string `json:"name"`
						Error string `json:"erroo"`
						Code  string `json:"codd"`
					}
