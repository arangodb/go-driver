...
const (
	...
	// queryImportOverwrite is the query parameter for POST /_api/import (replace collection data).
	// Document create uses [QueryOverwriteMode] instead.
	queryImportOverwrite = "overwrite"
)
...
func (c *CollectionDocumentImportOptions) modifyRequest(r connection.Request) error {
	...
	if c.Overwrite != nil {
		r.AddQuery(queryImportOverwrite, boolToString(*c.Overwrite))
	}
