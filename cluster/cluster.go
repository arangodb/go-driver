...
func (c *clusterConnection) Do(ctx context.Context, req driver.Request) (driver.Response, error) {
	...
	attempt := 1
	for {
		// Send request to specific endpoint with a 1/[1,2,3] timeout (so we get 1 or 2 or 3 attempts)
		serverCtx, cancel := context.WithTimeout(ctx, durationPerRequest)
		resp, err := server.Do(serverCtx, req)
		cancel()

		if err == nil && resp.StatusCode() == http.StatusServiceUnavailable && c.canFollowLeader(ctx) {
			// Service unavailable, handle the "no leader" error.
			var errArango driver.ArangoError

			if resp.ParseBody("", &errArango) == nil && errArango.HasError && driver.IsNoLeader(errArango) {
				attempt++
				if attempt > serverCount {
					// A specific server was specified or all servers failed. Give up.
					return nil, driver.WithStack(errArango)
				}
		...
		if req.Written() {
			// Request has been written to network, do not failover. The error must be returned.
		...
		// The request has not been written, try next server.
		attempt++
		if attempt > serverCount {
			// A specific server was specified or all servers failed. Give up.
			return nil, driver.WithStack(err)
...
// canFollowLeader returns true if searching leader is enabled.
func (c *clusterConnection) canFollowLeader(ctx context.Context) bool {
